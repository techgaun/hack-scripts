...
if is_debian; then
  ...
  apt install -y build-essential git vim silversearcher-ag axel aircrack-ng pyrit reaver wifite zenmap thc-ipv6 \
    ...
    exif exifprobe fluxgui neovim yarn fortune cowsay mpd mpc dstat htop libevent-dev clang-4.0 global exuberant-ctags \
...
npm i -g diff-so-fancy apidoc flow-bin git+https://github.com/ramitos/jsctags.git
